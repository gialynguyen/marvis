...
export class MemoryStore {
  ...
  constructor(dbPath = "data/marvis.db") {
  ...
  async addMessage(conversationId: string, message: ChatMessageInput): Promise<string> {
    const id = randomUUID();
    const now = Date.now();
    const content =
  ...
  async getTotalTokens(conversationId: string): Promise<number> {
    const row = this.db
      .prepare("SELECT SUM(tokens_estimated) as total FROM messages WHERE conversation_id = ?")
